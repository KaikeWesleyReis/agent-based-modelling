...
#define FATOR_AUMENTO_VISAO 0.05 //Fator aumento de visao de combate
#define DIST_MAX_COMBATE 0.3 //Dist para comecar a lutar
#define RODADA_MAX_ESTAGNADA 3
#define DIST_MAX_PERMITIDA_VISAO 0.99



//PUBLIC FUNCTIONS:
coliseu::coliseu(int SIZE_FIGHTERS)
{
    ...
    for(int i=0;i<tamLutadores;i++)
    {
        ...
        for(int j=0;j<tamLutadores;j++)
        {
            if(i != j)
            {
                ...
                if(diff <= this->DIST_PARA_COMBATE && diff < d)
                {
                    d = diff;
                    menor = j;
                }
        ...
        if(menor != -1) this->competidores[i].addOponente(&this->competidores[menor]);
        else this->competidores[i].addOponente(nullptr);
    }
}
...
{
    int tamLutadores = this->competidores.size();
    for(int i=0;i<tamLutadores;i++)//Agir
    {
        this->competidores[i].agir();
        this->competidores[i].cleanLutador();//evitar lixo de variaveis de pensamento
    }
    for(int i=0;i<tamLutadores;i++)//Matar remanescentes da rodada
    {
        if(this->competidores[i].getEhMorto()) this->competidores.erase(this->competidores.begin()+i);
    }
}
...
void coliseu::efeitoVisaoDeCombate()
{
    ...
    else
    {
        ...
        {
            if(this->DIST_PARA_COMBATE < DIST_MAX_PERMITIDA_VISAO) this->DIST_PARA_COMBATE += FATOR_AUMENTO_VISAO;
        }
